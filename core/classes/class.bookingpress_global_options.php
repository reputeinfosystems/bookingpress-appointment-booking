    ...
    class BookingPress_Global_Options Extends BookingPress_Core
    {
        ...
        {   
            ...
            $bpa_general_settings_data = array();
                ...
                // When the Pro plugin is INACTIVE, only the Lite-
                // supported formats are valid time slot tokens.  If the DB
                // still carries a Pro-only format that was saved while Pro
                // was active (anything other than `g:i a`, `H:i`, or the
                // explicit WP-inherit sentinel), the bucket renderer at
                // class.bookingpress.php L105-113 does not recognise it
                // and the moment-style converted format defaults to
                // `hh:mm`, losing the AM/PM marker.  Force-normalise to
                // `g:i a` (Lite's documented default) so the slot label
                // always reflects a Lite-known format.
                if ( ! $BookingPress->bpa_is_pro_active()
                    && $default_time_format !== 'g:i a'
                    && $default_time_format !== 'H:i'
                    && $default_time_format !== 'bookingpress-wp-inherit-time-format'
                ) {
                    $default_time_format = 'g:i a';
                }
