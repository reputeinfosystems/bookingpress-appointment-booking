...
<bp-ui-tab-pane class="bpa-tabs--v_ls__tab-item--pane-body" name="migration_tool_settings" label="conversion_tracking" data-tab_name="migration_tool_settings">
	<template #label>
	...
	<div class="bpa-general-settings-tabs--pb__card bpa-migration-settings-tabs--pbtab">
		<bp-ui-row type="flex" class="bpa-mlc-head-wrap-settings bpa-mlc-head-wrap-settings-migr bpa-gs-tabs--pb__heading">		   
		...
		<div class="bpa-gs--tabs-pb__content-body bpa-migration-settings-tabs--pb__card">
			<bp-ui-form method="post" ref="migration_tool_form" :model="migration_tool_form">
					...
					<div class="bpa-gs__cb--item">
						...
						<div class="bpa-gs__cb--item-body">							
							<bp-ui-row type="flex" class="bpa-gs--tabs-pb__cb-item-row">
								<bp-ui-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12" class="bpa-gs__cb-item-left">
									<h4><?php esc_html_e( 'Select Export Data', 'bookingpress-appointment-booking' ); ?></h4>
								</bp-ui-col>
								<bp-ui-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12" class="bpa-gs__cb-item-right">								  	
									<div class="bpa-migra-inner-field">
										<div :class="((continue_export_id != '' || export_complete_msg != '')?'bpa-export-inner-fields-cont':'')" class="export-inner-fields">
											...
											<div class="bpa-export-scroll-builder">
												<div class="bpa-export-scroll-header">	
													<div class="bpa-export-item">									
														<label class="bpa-form-label bpa-custom-checkbox--is-label bpa-custom-checkbox--is-label-tit"> 
															<bp-ui-checkbox @change="bpa_select_all_export_list($event)" v-model="export_all_record" label=""></bp-ui-checkbox> 
															<?php echo esc_html__('Select ALL','bookingpress-appointment-booking'); ?> 
														</label>
													</div>
												</div>
													...
													<div v-if="(migration_tool_form.export_list.length != 0)" class="bpa-export-list">
														<div v-for="(export_list,keys) in migration_tool_form.export_list" class="bpa-export-item">
															<label class="bpa-form-label bpa-custom-checkbox--is-label"> 
																<bp-ui-checkbox label="" :disabled="migaration_child_active(export_list,keys)" v-model="migration_tool_form.bookingpress_export_list_data[keys]" @change="bpa_select_export_list(export_list,$event)"></bp-ui-checkbox><span v-html="export_list.name"></span>  <span v-if="typeof export_list.total_record != 'undefined'">({{export_list.total_record}}) </span>
															</label>
															<template v-if="export_list.child.length != 0">
																<div v-for="(export_list_child,child_keys) in export_list.child" class="bpa-export-item bpa-export-item-child">										
																	<label class="bpa-form-label bpa-custom-checkbox--is-label"> 
																		<bp-ui-checkbox label="" :disabled="migaration_child_active(export_list_child,child_keys)" v-model="migration_tool_form.bookingpress_export_list_data[child_keys]" @change="bpa_select_export_list(export_list_child,$event,keys)"></bp-ui-checkbox> 
																		<span v-html="export_list_child.name"></span> <span v-if="typeof export_list_child.total_record != 'undefined'">({{export_list_child.total_record}}) </span>
																	</label>
																</div>
															</template>
														</div>
											...
											<div v-if="export_log_data.length != 0 || export_complete_msg != ''" class="bpa-export-data-log">
												<div class="bpa-field-outer-container">
													...
													<template v-if="export_log_data.length != 0">
														<div v-for="export_log_single in export_log_data" class="bpa-cfs-ic__body">							
															...
															<div class="bpa-cfs-ic--body">
																<template v-for="export_log_single_data in export_log_single.export_detail" :key="export_log_single_data.label">
																	<div v-if="export_log_single_data.export_detail_record_hide == '0'"  class="bpa-cfs-ic--body__field-preview">
																		<div class="export-log-row">
																			<div class="export-log-row-single">
																				<span class="export-log-data-type">{{export_log_single_data.label}}</span> 
																				<span class="export-log-data-total">({{export_log_single_data.export_detail_total_record}}/{{export_log_single_data.export_detail_last_record}})</span>
																			</div>
																		</div>
																	</div>
																</template>
											...
											<bp-ui-button v-if="(continue_export_id == '' || continue_export_id == '0') && export_last_download_file == ''" class="bpa-btn bpa-btn--primary bpa-btn__medium bpa-btn__export_data" @click="bookingpress_export_data_task()" :class="(is_display_export_loader == '1' || continue_export_id != '') ? 'bpa-btn--is-loader' : ''" :disabled="(is_display_export_loader == '1')?true:false">                    
										...
										<bp-ui-button v-if="continue_export_id != '' && continue_export_id != '0'" class="bpa-btn bpa-btn--primary bpa-btn__medium bpa-btn--danger bpa-btn__export_data" :class="(is_display_export_loader == '1') ? 'bpa-btn--is-loader' : ''" @click="bookingpress_stop_export_process()">                    
										...
										<div v-if="export_last_download_file != ''" class="export-file"> 
											<a class="bp-ui-button bpa-btn bpa-btn--primary bpa-btn__medium bpa-btn__export_data bp-ui-button--default bpa-ex-download" :href="export_last_download_file" download><?php esc_html_e( 'Download File', 'bookingpress-appointment-booking' ); ?></a>
										</div>	
									</div>	
								</bp-ui-col>
							</bp-ui-row>												
									...
									<div class="bpa-migra-inner-field">	
										<div :class="(continue_import_id != ''?'bpa-export-inner-fields-cont':'')" class="export-inner-fields">
											<div class="import-inner-field-box">													
												<bp-ui-input class="bpa-form-control bpa-import-txt-box" placeholder="<?php esc_html_e('Copy and paste the exported file content', 'bookingpress-appointment-booking'); ?>" type="textarea" :rows="20" v-model="migration_tool_form.import_data"></bp-ui-input>						
												<bp-ui-button @click="bookingpress_import_data_task()" :class="(is_display_import_loader == '1') ? 'bpa-btn--is-loader' : ''" :disabled="(continue_import_id != '')?true:false" class="bpa-btn bpa-btn--primary bpa-btn__medium bpa-btn__export_data">                    
